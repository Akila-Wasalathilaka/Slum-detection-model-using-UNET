...
def train_epoch(model, train_loader, criterion, optimizer, device, config, epoch):
    ...
    for batch_idx, (images, masks) in enumerate(train_loader):
        ...
        is_multiclass = hasattr(train_loader.dataset, 'mode') and train_loader.dataset.mode == 'multiclass'
        ...
        with torch.no_grad():
            if is_multiclass:
                slum_idx = 1 if getattr(train_loader.dataset, 'classes', ['background','slum'])[1] == 'slum' else 1
                slum_logits = outputs[:, slum_idx:slum_idx+1]
                slum_target = (masks == slum_idx).float().unsqueeze(1)
                metrics = metrics_calc(slum_logits, slum_target)
            else:
...
def validate_epoch(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for images, masks in val_loader:
            ...
            is_multiclass = hasattr(val_loader.dataset, 'mode') and val_loader.dataset.mode == 'multiclass'
            if not is_multiclass:
                if len(masks.shape) == 3:
                    masks = masks.unsqueeze(1)
                masks = masks.float()
            ...
            # Calculate metrics
            if is_multiclass:
                slum_idx = 1 if getattr(val_loader.dataset, 'classes', ['background','slum'])[1] == 'slum' else 1
                slum_logits = outputs[:, slum_idx:slum_idx+1]
                slum_target = (masks == slum_idx).float().unsqueeze(1)
                metrics = metrics_calc(slum_logits, slum_target)
            else:
...
def main():
    ...
    mode=getattr(data_config, 'mode', 'binary'),
    class_rgb_map=getattr(data_config, 'class_rgb_map', None)
    )
    ...
    # Determine in/out channels
    in_channels = getattr(data_config, 'input_channels', 3)
    if getattr(data_config, 'mode', 'binary') == 'multiclass':
        # Number of classes equals number of entries in class_rgb_map
    ...
    # Choose loss
    if getattr(data_config, 'mode', 'binary') == 'multiclass':
        ce_weights = class_weights.get('ce_weights') if isinstance(class_weights, dict) else None
        weight_tensor = torch.tensor(ce_weights, dtype=torch.float32).to(device) if ce_weights else None
